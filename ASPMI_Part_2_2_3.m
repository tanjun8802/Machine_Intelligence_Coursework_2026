...
% 
% figure;
% plot(n_vec, s,          'Color', [0.7 0.7 0.7], 'LineWidth', 0.8); hold on;
% plot(n_vec, x_hat_demo, 'b',                     'LineWidth', 1.2); % ALE
% plot(n_vec, x_hat_anc,  'r',                     'LineWidth', 1.2); % ANC
% plot(n_vec, x_clean,    'k--',                   'LineWidth', 1.2); % clean
% 
% xlabel('n');
% ylabel('Amplitude');
% title(sprintf('ANC vs ALE (M_{ANC}=5, M_{ALE}=%d, \\Delta=%d, \\mu=0.01)', M_demo, Delta_min));
% legend({'Noisy s(n)', 'ALE output \hat{x}_{ALE}(n)', ...
%         'ANC output \hat{x}_{ANC}(n)', 'Clean x(n)'}, ...
%        'Location', 'best');
% grid on;
%% =========================================================
...
if ~isfile(eeg_file)
    ...
    mu_eeg  = 0.001;
    M_eeg   = 10;   % Longer filter to handle phase offset of 50 Hz
